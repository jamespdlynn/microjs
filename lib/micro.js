(function(definition) {
     ...
     * @param {string} [name] The name of the schema being registered
     * @param {boolean} [serializeType=true] Flag determining whether or not to serialize the typeId along with the data
     * @param {number} [typeId] A single byte integer used as a type identifier when serializing (defaults to the current number of serialized schemas)
     */
    MicroSerializer.prototype.register = function(schema, name, serializeType, typeId){
        ...
        if (typeof name !== 'string'){
            ...
            for (var key in schema){
               ...
               serializeType = (typeof serializeType === 'boolean') ? serializeType : true;
               typeId = (typeof typeId == 'number') ? index : this._schemaNames.length;

                if (serializeType && (typeId%1 > 0 || typeId < 0 || typeId > 255)){
                    throw new Error("Error parsing schema '"+name+"' : Invalid typeId '"+typeId+"' (must be an integer between 0-255");
                }
            ...
            try{
                ...
                if (serializeType){
                    this._schemaNames[typeId] = name;
                }
            }
            catch (e){
                throw new Error("Error parsing schema '"+name+"' :"+ e.message);
            }
     ...
     * @param {object} json The JSON data object to be serialized
    ...
    MicroSerializer.prototype.toBinary = function(json, schemaName, maxBytes){
        ...
        if (!schemaName){
            throw new Error("Must specify a schema name");
        }
    ...
    function readFromBuffer(schema, buffer, data, returnByteLength){
        ...
        for (var i=0; i < schema.length; i++){
            ...
            switch (type){
                ...
                case "object":
                    ...
                    byteLength = byteLength || value._byteLength;

                    delete value._byteLength;
                    data[property.name] = value;
